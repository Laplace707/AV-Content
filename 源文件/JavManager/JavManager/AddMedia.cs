...
namespace JavManager
{
    ...
    public partial class Form2 : Form
    {
        ...
        private void openFileDialog2_FileOk(object sender, CancelEventArgs e)
        {
            ...
            if (pan.Equals(filepath.Substring(0, 1)))
            {
                ...
                if (checkBox1.Checked) {
                    GetMessageFromName(vediodictxt.Text);
                }
            }
            else
            {
        ...
        {
            if (!vediodictxt.Text.Equals(string.Empty)) {
                GetMessageFromName(vediodictxt.Text);
            }
        }


        private void GetMessageFromName(String path) {
            string[] name = path.Split('\\');
            string[] messages = name[name.Length - 1].Split('.')[0].Split('_');
            string[] comno = messages[0].Split('-');
            string[] actress = messages[1].Split(' ');
            comtxt.Text = messages[0].Split('-')[0];
            notxt.Text = messages[0].Substring(messages[0].Split('-')[0].Length + 1);
            nametxt.Text = messages[2];
            playertxt.Text = string.Join(";", actress) + ";";
        }

        private void Button2_Click(object sender, EventArgs e)
        {
            AVSearch aVSearch = new AVSearch();
            aVSearch.ShowDialog();
            if (data.code != null && !data.code.Equals(string.Empty)) {
                string code = data.code;
                if (code.Contains('-'))
                {
                    comtxt.Text = code.Split('-')[0];
                    notxt.Text = code.Split('-')[1];
                }
                else if (code.Contains(' ')) {
                    string[] comno = code.Split(' ');
                    string com = "";
                    for (int i = 0; i < comno.Length -1; i++) { com = com + comno[i]; }
                    comtxt.Text = com;
                    notxt.Text = comno[comno.Length - 1];
                }
                nametxt.Text = data.title;
                playertxt.Text = data.actresses;
                tagtxt.Text = data.genres;
            }
        }
    }
}
